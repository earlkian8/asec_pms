    ...
    public function projectDetail(Request $request, $id)
    {
        ...
        $formattedLaborCosts = $project->laborCosts->map(function ($laborCost) {
            return [
                ...
                'basePay' => (float) ($laborCost->base_pay ?? 0),
                'overpayAmount' => (float) ($laborCost->overpay_amount ?? 0),
                'overpayReason' => (string) ($laborCost->overpay_reason ?? ''),
                'doublePayAmount' => (float) ($laborCost->double_pay_amount ?? 0),
                'doublePayReason' => (string) ($laborCost->double_pay_reason ?? ''),
                'damagesDeduction' => (float) ($laborCost->damages_deduction ?? 0),
                'damagesReason' => (string) ($laborCost->damages_reason ?? ''),
                'otherDeduction' => (float) ($laborCost->other_deduction ?? 0),
                'otherDeductionReason' => (string) ($laborCost->other_deduction_reason ?? ''),
                'cashAdvance' => (float) ($laborCost->cash_advance ?? 0),
                'cashAdvanceReason' => (string) ($laborCost->cash_advance_reason ?? ''),
                'deductionItems' => (array) ($laborCost->deduction_items ?? []),
                'payrollEvents' => (array) ($laborCost->payroll_events ?? []),
