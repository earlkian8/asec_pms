...
import { Trash2, Filter, X, Search, Calendar, MessageSquare, ArrowUpDown, AlertCircle, FileText } from 'lucide-react';
...
export default function RequestUpdatesIndex() {
  ...
  const activeFiltersCount = () => {
    let count = 0;
    if (localFilters.client_id && localFilters.client_id !== 'all') count++;
    if (localFilters.project_id && localFilters.project_id !== 'all') count++;
  ...
  const applyFilters = (e) => {
    ...
    try {
      ...
      router.get(route('client-management.request-updates.index'), params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onSuccess: () => {
  ...
  const resetFilters = () => {
    ...
    setSortOrder('desc');
    router.get(route('client-management.request-updates.index'), { search: searchInput }, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
      onSuccess: () => {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
      router.get(
        route('client-management.request-updates.index'),
        { search: searchInput },
            ...
            <div className="mb-6 pb-6 border-b border-gray-200">
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg p-4 border border-amber-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-medium text-amber-700 uppercase tracking-wide">Unique Clients</p>
                      <p className="text-2xl font-bold text-amber-900 mt-1">
                        {new Set(requests.map(r => r.client_id)).size}
                      </p>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6 items-center justify-between relative z-50">
              <div className="flex flex-col sm:flex-row gap-3 items-center flex-1 relative z-50">
                <div className="relative flex-1 max-w-md">
                  ...
                  <Input
                    placeholder="Search requests by subject, message, client, or project..."
                    value={searchInput}
                      ...
                      >
                        <div className="bg-gradient-to-r from-zinc-700 to-zinc-800 px-4 py-3 flex items-center justify-between flex-shrink-0">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-base font-semibold text-white">Filter Requests</h3>
                          </div>
                          <button
                                ...
                                <SelectContent>
                                  ...
                                  {filterOptions.clients.map((client) => (
                                    <SelectItem key={client.id} value={client.id.toString()}>
                                      {capitalizeText(client.client_name)} ({client.client_code})
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                                ...
                                <SelectContent>
                                  ...
                                  {filterOptions.projects.map((project) => (
                                    <SelectItem key={project.id} value={project.id.toString()}>
                                      {capitalizeText(project.project_name)} ({project.project_code})
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                      ...
                      >
                        <div className="bg-gradient-to-r from-zinc-700 to-zinc-800 px-4 py-3 flex items-center justify-between flex-shrink-0">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-base font-semibold text-white">Sort Requests</h3>
                          </div>
                          <button
                    ...
                    requests.map((request, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-left px-4 py-4 text-sm text-gray-700">
                          {request.project ? (
                            <div>
                              <span className="font-medium">{capitalizeText(request.project.project_name)}</span>
            ...
            {showPagination && (
              <div className="flex flex-col sm:flex-row items-center justify-between mt-6 pt-6 border-t border-gray-200 gap-4">
                <div className="text-sm text-gray-600">
                  Showing <span className="font-semibold text-gray-900">{requests.length}</span> of{' '}
                  <span className="font-semibold text-gray-900">{pagination?.total || 0}</span> requests
                </div>
