...
const EditEmployee = ({ setShowEditModal, employee }) => {
  ...
  const [avatarPreview, setAvatarPreview] = useState(employee?.profile_image_url || null);
  const avatarRef = useRef(null);
  const compensation = employee?.resolved_compensation || {};
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    post(route('employee-management.update', employee.id), {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: (page) => {
        ...
        else toast.success(`Employee "${employee.first_name} ${employee.last_name}" updated successfully`);
      },
      onError: () => {
        ...
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-600">
            ...
            {employee?.employee_id && (
              <span className="ml-2 text-xs font-mono text-zinc-500 bg-zinc-100 px-2 py-0.5 rounded">
                {employee.employee_id}
              </span>
            )}
          </DialogDescription>
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-4">
            ...
            {activeTab === 'govids' && (
              <div className="grid grid-cols-1 gap-3">
                ...
                <GovIdRow label="SSS"        numberKey="sss_number"        imageKey="sss_id_image"        data={data} setData={setData} errors={errors} existingImageUrl={employee?.sss_id_image_url} />
                <GovIdRow label="PhilHealth" numberKey="philhealth_number"  imageKey="philhealth_id_image" data={data} setData={setData} errors={errors} existingImageUrl={employee?.philhealth_id_image_url} />
                <GovIdRow label="Pag-IBIG"   numberKey="pagibig_number"     imageKey="pagibig_id_image"    data={data} setData={setData} errors={errors} existingImageUrl={employee?.pagibig_id_image_url} />
                <GovIdRow label="TIN"        numberKey="tin_number"         imageKey="tin_id_image"        data={data} setData={setData} errors={errors} existingImageUrl={employee?.tin_id_image_url} />
              </div>
            )}
