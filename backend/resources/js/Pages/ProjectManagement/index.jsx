...
import { PROJECT_LIST_COLUMNS, STATUS_COLORS, PRIORITY_COLORS } from '@/constants/projectManagement';
...
export default function ProjectsIndex() {
  ...
  const columns = PROJECT_LIST_COLUMNS;
                  ...
                  {projects.length > 0 ? (
                    projects.map((project, index) => {
                      ...
                      >
                        ...
                        <TableCell className="text-left px-4 py-4 text-sm">
                          <span className={`px-3 py-1.5 rounded-full text-xs font-medium ${STATUS_COLORS[project.status] || 'bg-gray-100 text-gray-800 border border-gray-200'}`}>
                            {capitalizeText(project.status?.replace('_', ' ') || '')}
                          </span>
                        </TableCell>
                        <TableCell className="text-left px-4 py-4 text-sm">
                          <span className={`px-3 py-1.5 rounded-full text-xs font-medium ${PRIORITY_COLORS[project.priority] || 'bg-gray-100 text-gray-800 border border-gray-200'}`}>
